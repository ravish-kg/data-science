...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    new_weights = np.insert(w, 0, b, axis=0)
    y = np.where(y == 0, -1, 1)
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations + 1):
            prediction_bool_matrix = binary_predict(new_x, new_weights, 10101010) == 0
            prediction_matrix = np.where(prediction_bool_matrix, -1, 1)
            y_w_transpose_x = np.where((y) * prediction_matrix <= 0, 1, 0)
            new_weights = new_weights + (get_eeta(step_size, N) * (np.dot(y_w_transpose_x * y, new_x)))
        ...
        ################################################
        for i in range(max_iterations + 1):
            sigmoid_output = np.dot(sigmoid(-(y * np.dot(new_x, new_weights))) * y, new_x)
            new_weights = new_weights + get_eeta(step_size, N) * sigmoid_output
...
def binary_predict(X, w, b):
    ...
    prediction = np.where(np.sign(np.dot(X, w)) == -1, 0, 1)
